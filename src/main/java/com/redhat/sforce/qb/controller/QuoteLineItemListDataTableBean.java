...
public class QuoteLineItemListDataTableBean {
	...
	public void validateProduct(AjaxBehaviorEvent event) {
		...
		int rowIndex = Integer.valueOf(event.getComponent().getAttributes()
				.get("rowIndex").toString());

		QuoteLineItem quoteLineItem = quoteController.getSelectedQuote()
				.getQuoteLineItems().get(rowIndex);
		try {
			PricebookEntry pricebookEntry = queryPricebookEntry(quoteController
					.getSelectedQuote().getPricebookId(), productCode,
					quoteController.getSelectedQuote().getCurrencyIsoCode());

			quoteLineItem.setBasePrice(0.00);
			quoteLineItem.setListPrice(pricebookEntry.getUnitPrice());
			quoteLineItem.setDescription(pricebookEntry.getProduct()
					.getDescription());
			...
			ResourceBundle resource = ResourceBundle.getBundle(
					"com.redhat.sforce.qb.resources.messages", context
							.getViewRoot().getLocale());
			FacesMessage message = new FacesMessage(
					FacesMessage.SEVERITY_ERROR, null, MessageFormat.format(
							resource.getString("invalidSKU"), productCode));
			FacesContext.getCurrentInstance().addMessage(
					inputText.getClientId(context), message);
		}
	}

	private PricebookEntry queryPricebookEntry(String pricebookId,
			String productCode, String currencyIsoCode)
			throws SalesforceServiceException {
		return sessionManager.queryPricebookEntry(pricebookId, productCode,
				currencyIsoCode);
	}
}