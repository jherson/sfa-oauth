...
public class QuoteLineItemScheduleFactory {
	...
	public static List<QuoteLineItemSchedule> deserialize(JSONArray jsonArray)
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			quoteLineItemSchedule.setProrateUnitPrice(wrapper
					.getDouble("ProrateUnitPrice__c"));
			quoteLineItemSchedule.setProrateTotalPrice(wrapper
					.getDouble("ProrateTotalPrice__c"));
			quoteLineItemSchedule.setProrateYearUnitPrice(wrapper
					.getDouble("ProrateYearUnitPrice__c"));
			quoteLineItemSchedule.setProrateYearTotalPrice(wrapper
					.getDouble("ProrateYearTotalPrice__c"));
			...
			quoteLineItemSchedule.setPricePerDay(wrapper
					.getDouble("PricePerDay__c"));
			quoteLineItemSchedule.setYear(wrapper.getInteger("Year__c"));
			quoteLineItemSchedule.setEndDate(wrapper.getDate("EndDate__c"));
			quoteLineItemSchedule.setQuoteLineItem(QuoteLineItemFactory
					.deserialize(wrapper.getJSONObject("QuoteLineItemId__r")));
